...
let s:indent_spaces = get(g:, 'airline#extensions#whitespace#indent_spaces', 2)
  ...
  if !exists('b:airline_whitespace_check')
    ...
    if index(checks, 'indent') > -1
      ...
      if indents[0] != 0 && indents[2] != 0
        let mixed = indents[0] == indents[1] ? indents[0] : (indents[2] == indents[3] ? indents[2] : min(indents))
      else
        let mixed = search(spaces . '\t')
      endif
    endif
